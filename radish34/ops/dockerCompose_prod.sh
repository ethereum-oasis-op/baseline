...
function dotEnv {
  ...
  elif [[ -f env.prod.env && -n "`extractEnv $key env.prod.env`" ]] # Third choice: load from public defaults
  then extractEnv $key env.prod.env
  fi
}
...
echo "Creating /config/docker-compose.yml file..."
mkdir -p config/
