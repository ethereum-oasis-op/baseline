...
const MSACard = ({ msa, price, onClick, selected, volume }) => {
  ...
  const valueForNormalise = () => {
    if (volume && !accumulatedVolumeOrdered) return Number(volume);
    if (!volume && accumulatedVolumeOrdered) return Number(accumulatedVolumeOrdered);
    if (volume && accumulatedVolumeOrdered) return Number(volume) + Number(accumulatedVolumeOrdered);
    else return 0;
  };
  const linearProgressValue = normalise(valueForNormalise(), msa.tierBounds[0], msa.tierBounds[msa.tierBounds.length - 1]);
  const invalidAmount = volume ? Number(volume) + Number(accumulatedVolumeOrdered) > msa.tierBounds[msa.tierBounds.length - 1] : false;
      ...
      <Card className={clsx(classes.root, {
        ...
        <CardActionArea onClick={onClick} disabled={latestCommitment.index === null}>
          ...
          <CardContent style={{ background: "#EAF4FF", maxWidth: 345, height: 100 }}>
            <Typography variant="overline">ESTIMATED COST FOR ORDER</Typography>
