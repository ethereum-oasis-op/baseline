...
describe('Whisper Identities', () => {
  test('should create a new buyer identity buyer', async () => {
    const res = await request(buyerURL)
      .post('/graphql')
      .send({ query: query });
    expect(res.statusCode).toEqual(200);
    expect(res.body.data.createIdentity).toHaveProperty('publicKey');
    buyerId = res.body.publicKey;
  });

  test('should create a new supplier identity for supplier', async () => {
