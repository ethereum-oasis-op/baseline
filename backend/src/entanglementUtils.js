...
async function createEntanglement(messenger, doc) {
  ...
  doc.partnerIds.forEach(async (partnerId) => {
    await messenger.sendPrivateMessage(myId, partnerId, undefined, JSON.stringify(entangleRequest));
  });
  return result;
}
...
// Get all Entanglement objects for the given user
async function getSingleEntanglement(query) {
...
async function updateEntanglement(senderId, messageObj) {
...
async function selfUpdateEntanglement(messenger, entanglementDoc, collectionName, docId) {
  ...
  entanglementDoc.participants.forEach(async ({ messengerId, isSelf }) => {
    ...
    if (!isSelf) {
      await messenger.sendPrivateMessage(myMessengerId, messengerId, undefined, JSON.stringify(entangleMessage));
    }
  });
  return updated;
}
...
async function acceptEntanglement(messenger, entanglementId, doc) {
  if (!doc.messengerId) {
  ...
  entangleObject.participants.forEach(async ({ messengerId, isSelf }) => {
    // Don't send message to self
    if (!isSelf) {
      await messenger.sendPrivateMessage(doc.messengerId, messengerId, undefined, JSON.stringify(entangleMessage));
    }
  });
  return result;
}
...
async function calculateHash(collectionName, docId) {
  let entangledObject;
  switch (collectionName) {
    case 'RFPs':
      entangledObject = await RFP.findOne({ _id: docId });
      break;
    default:
