...
export const getPOById = async id => {
  try {
    ...
    logger.error('Error getting PO from db.\n%o', err, { service: 'API' });
    return false;
  }
};
...
export const savePO = async poObject => {
  try {
    ...
    logger.info(`Saved PO to db:\n%o`, doc[0].toObject(), { service: 'API'});
    return doc[0].toObject();
  } catch (err) {
    logger.error('Error saving PO in db.\n%o', err, { service: 'API' });
    return false;
  }
};
...
export const updatePOWithNewCommitment = async poObject => {
  ...
  if (!exists) {
    logger.error(`PO does not exist for this msaId ${poObject._id}. Hence cannot be updated.`, { service: 'API' });
  ...
  try {
    ...
    logger.info(`Updated PO with new commitment:\n%o`, doc, { service: 'API'});
    return doc;
  } catch (err) {
    logger.error('Error updating PO in db.\n%o', err, { service: 'API' });
    return false;
  }
};
...
export const updatePOWithCommitmentIndex = async poObject => {
  ...
  try {
    ...
    logger.info(`Updated PO with new commitment index:\n%o`, doc, { service: 'API'});
    return doc;
  } catch (err) {
    logger.error('Error updating PO in db.\n%o', err, { service: 'API' });
    return false;
...
export const createPO = async (_zkpPrivateKeyOfBuyer, oldMSA, newMSA, po) => {
  ...
  logger.info('Create PO TX complete.', { service: 'API'});
