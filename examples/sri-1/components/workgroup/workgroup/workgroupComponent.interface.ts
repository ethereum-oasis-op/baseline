...
import { IBpiSubject } from '../../identity/bpiSubject/bpiSubject.interface';
...
export interface IWorkgroupComponent {
    ...
    createWorkgroup(name: string, id: string, owner: IBpiSubject, worksteps: Workstep[]): Workgroup;  //[R236] There MUST be at least one BPI Subject role that has the authorization to create a workgroup. [R237] A workgroup MUST consist of at least one BPI Subject participant.
    getWorkgroupById(id: string): Workgroup
    sendInviteToWorkgroup(id: string, name: string, sender: IBpiSubject, recipient: string, workgroupId: string, agreement: Agreement): Invitation;
