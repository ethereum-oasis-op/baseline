...
func main() {
	...
	err = workflowTx.VaultSign(tokenStr, vaultID, keyID)
	if err != nil {
		panic(err)
	}

	workflowID, err := workflowTx.Broadcast()
	if err != nil {
		panic(err)
	}
	...
	err = workflowNotificationTx.VaultSign(tokenStr, vaultID, keyID)
	if err != nil {
		panic(err)
	}
