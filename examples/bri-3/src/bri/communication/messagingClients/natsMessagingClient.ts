...
@Injectable()
export class NatsMessagingClient implements IMessagingClient, OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    try {
      ...
      this.log.logWarn(`Error: ${err} connecting to nats server at: ${process.env.BPI_NATS_SERVER_URL}`);
    }
  }
