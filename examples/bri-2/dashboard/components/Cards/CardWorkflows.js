...
const statusIcon = (status) => {
  switch (status) {
    ...
    case 'closed':
      return <><i className="fas fa-circle text-red-500 mr-2"></i>{" "}{status}</>;
  ...
  switch (clientType) {
    case 'dashboard-test':
      return (<div style={{
                ...
                <img src={require("assets/img/brand/baselineLogoMark-FullColor.png")}
                  alt="baseline icon"
                  style={{
                    paddingRight: "9px",
                    width: "30px"
...
export default function CardWorkflows({ workflows, isLoading, isError, color }) {
  return (
    <>
      <div
        className={
            ...
            <tbody>
              { (workflows && workflows.length > 0)
                ? workflows.map(( workflow, index ) => {
                  return (
                    <tr key={workflow._id} style={{"borderTop":"solid 1px rgba(105, 105, 105)"}}>
