...
import { unsubscribeMerkleEvents } from './blockchain';
...
import { verifyAndPush, trackShield } from './route-handlers/rpc-methods';
...
const baseline_verifyAndPush = new jayson.Method(
  async (args: any, context: any, done: any) => {
    ...
    const error = validateParams(args, 7);
    if (error) {
      done(error, null);
      return;
    }
    ...
    const proofB = args[3];
    const proofC = args[4];
    const publicInputs = args[5];
    const newCommitment = args[6];
...
const baseline_track = new jayson.Method(
  async (args: any, context: any, done: any) => {
    ...
    const contractAddress = args[0];
    const result = await trackShield(contractAddress);
