...
export const unmarshalProtocolMessage = (msg: Buffer): Message => {
  ...
  const type = msg.subarray(5 + 42 + 42 + 36 + 64 + reservedSize, 5 + 42 + 42 + 36 + 64 + 1 + reservedSize).toString();
  return {
    opcode: msg.subarray(0, 5).toString().replace(/\0/g, ''),
    recipient: msg.subarray(5, 5 + 42).toString(),
    shield: msg.subarray(5 + 42, 5 + 42 + 42).toString(),
    identifier: msg.subarray(5 + 42 + 42, 5 + 42 + 42 + 36).toString(),
    signature: msg.subarray(5 + 42 + 42 + 36 + reservedSize, 5 + 42 + 42 + 36 + 64 + reservedSize).toString(),
    type: type === '0' ? PayloadType.Binary : (type === '1' ? PayloadType.Text : null),
    payload: msg.subarray(5 + 42 + 42 + 36 + 64 + reservedSize + 1),
  } as Message;
};
